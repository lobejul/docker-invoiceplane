...
```bash
docker build -t lobejul/invoiceplane github.com/lobejul/docker-invoiceplane
```
...
docker run --name invoiceplane -itd --restart=always \
  ...
  lobejul/invoiceplane:1.5.9 app:invoiceplane
```
...
docker run --name invoiceplane-nginx -itd --restart=always \
  ...
  --publish 10080:80 \
  lobejul/invoiceplane:1.5.9 app:nginx
```
...
docker run --name invoiceplane -it --rm [OPTIONS] \
  lobejul/invoiceplane:1.5.9 app:backup:create
```
...
```bash
docker run --name invoiceplane -it --rm [OPTIONS] \
  lobejul/invoiceplane:1.5.9 app:backup:restore
```
...
```bash
docker run --name invoiceplane -it --rm [OPTIONS] \
  lobejul/invoiceplane:1.5.9 app:backup:restore BACKUP=1417624827_invoiceplane_backup.tar
```
